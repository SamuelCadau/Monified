 ...
 */
  async sendEmail(token, email, id) {
    ...
    if (!envConfig.mail.host || !envConfig.mail.user) {
      logger.warn("SMTP configuration missing, skipping password reset email.");
      return {
        accepted: [],
        rejected: [email],
    ...
    const baseUrl = envConfig.mail.frontendBaseUrl.replace(/\/$/, "");
    const link = `${baseUrl}/#/reset-password/users/${id}/${token}`;

    const options = {
      from: envConfig.mail.from,
      to: email,
      subject: "Reset password in front",
      text: `Hello, you can reset your password by clicking on the following link: ${link}`,
    };
